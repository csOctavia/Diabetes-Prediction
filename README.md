...
Little's MCAR test was used to check whether the missing data is MCAR. The test statistic is a chi-squared value that proposes a null hypothesis that the missing data is Missing Completely At Random. The p-value we got was 1.28 × 10^-9, less than 0.05, so we interpret it as being that the missing data is not MCAR19.
